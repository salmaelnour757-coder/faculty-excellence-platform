      ...
      <div style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between',
          ...
          <span style={{ fontSize:13, color:'#64748B' }}>
            {totalRated} of {totalItems} items rated · {progress}%
          </span>
        </div>
        <div style={{ background:'#DDE3EF', borderRadius:4,
                      height:6, overflow:'hidden' }}>
          <div style={{ height:'100%', borderRadius:4,
                        background:'#1A7B8C', width:`${progress}%`,
                        transition:'width .4s' }} />
        </div>
        ...
        {domainItems.map((item, idx) => (
          <div key={item.id} style={{
            ...
            {['importance','competence','priority'].map(scale => {
              ...
              return (
                <div key={scale} style={{
                  ...
                  <div style={{
                    width:130, fontSize:12, fontWeight:700,
            ...
            {ratings[`${item.id}_importance`] &&
             ratings[`${item.id}_competence`] &&
             ratings[`${item.id}_priority`] && (() => {
              const gap = ratings[`${item.id}_importance`]
                        - ratings[`${item.id}_competence`]
              ...
              const band = tni >= 13
                ...
                : tni >= 5
                ? { label:'Moderate', color:'#CA8A04', bg:'#FEF9C3' }
        ...
        {currentDomain < domains.length - 1 ? (
          ...
          <button onClick={handleSubmit} disabled={saving}
            style={{
              ...
              border:'none', background:'#C9982A',
              color:'#0D2B5E', fontWeight:700,
              fontSize:14, cursor:'pointer'
            }}>
            ✓ Submit Assessment
          </button>
